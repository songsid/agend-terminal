...
pub fn init_from_config(
    ...
    let token = match std::env::var(bot_token_env) {
        Ok(t) => t,
        Err(_) => {
            tracing::info!(env = %bot_token_env, "discord bot token not set, skipping");
            return None;
        }
    };
    ...
    let allowlist: Option<Vec<u64>> = user_allowlist
        .as_ref()
        .map(|list| list.iter().filter_map(|s| parse_snowflake(s, "user_allowlist").ok()).collect());

    let http = Arc::new(Http::new(&token));
    let gid = GuildId::new(guild_id_u64);
    let cat_name = category_name.clone();

    let (category_id, ch_map, inst_map) =
        match discord_runtime().block_on(setup_channels(Arc::clone(&http), gid, &cat_name, config, home)) {
            Ok(v) => v,
            Err(e) => {
...
async fn setup_channels(
        ...
        .find(|c| c.kind == ChannelType::Category && c.name == category_name)
    {
        ...
        guild_id
            .create_channel(
                &http,
                serenity::all::CreateChannel::new(category_name).kind(ChannelType::Category),
            )
            .await?
            .id
    };
    ...
    for name in config.instances.keys() {
        ...
        let cid = if let Some(cid) = existing_cid {
            ...
            if let Some(ch) = channels.values().find(|c| {
                c.kind == ChannelType::Text && c.parent_id == Some(category_id) && c.name == "general"
            }) {
                ch.id.get()
            } else {
...
impl EventHandler for Handler {
    async fn message(&self, _ctx: Context, msg: Message) {
        ...
        // Download attachments (Discord CDN URLs expire)
        ...
        tracing::info!(from = %username, to = %instance_name, %full_text, "discord inbound");
        ...
        // Save inbound message ID for status reactions
            ...
            rt.block_on(async {
                ...
                let mut client =
                    match serenity::Client::builder(&token, intents).event_handler(handler).await {
                        Ok(c) => c,
                        Err(e) => {
                            tracing::error!(error = %e, "discord client build failed");
                            return;
                        }
                    };
    ...
    match &config.channel {
        Some(ChannelConfig::Discord {
            ...
            let token = std::env::var(bot_token_env)
                .map_err(|_| anyhow::anyhow!("discord token env not set"))?;
...
impl crate::channel::Channel for DiscordChannel {
    ...
    fn create_binding(
        ...
        match create_channel_for_instance(&home, name) {
            Some(cid_str) => {
                let cid = cid_str
                    .parse::<u64>()
                    .map_err(|_| anyhow::anyhow!("invalid channel_id"))?;
    ...
    fn remove_binding(&self, binding: &BindingRef) -> anyhow::Result<()> {
        let payload = binding
            ...
            .ok_or_else(|| anyhow::anyhow!("non-discord binding"))?;
        let home = lock_state(&self.state).home.clone();
